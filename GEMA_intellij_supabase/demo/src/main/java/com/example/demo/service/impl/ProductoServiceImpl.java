...
public class ProductoServiceImpl implements ProductoService {
    ...
    public Producto actualizarProducto(String id, ProductoDto dto) {
        ...
        // 4. Actualizar Almacén (Movimiento de Inventario)
        if (dto.getIdAlmacen() != null) {
            List<Inventario> inventarios = inventarioRepo.findByProductoIdProducto(id);
            if (!inventarios.isEmpty()) {
                Inventario inv = inventarios.get(0); // Asumimos 1 ubicación por producto por ahora
                if (!inv.getAlmacen().getIdAlmacen().equals(dto.getIdAlmacen())) {
                ...
                // Caso raro: Producto existe pero no tiene inventario. Lo creamos.
                Almacen nuevoAlmacen = almacenRepo.findById(dto.getIdAlmacen())
                        .orElseThrow(() -> new RuntimeException("Almacen no encontrado"));
                Inventario inv = new Inventario();
                inv.setProducto(p);
                inv.setAlmacen(nuevoAlmacen);
                inv.setCantidadActual(dto.getCantidad() != null ? dto.getCantidad() : 0);
                inventarioRepo.save(inv);
            }
        }
