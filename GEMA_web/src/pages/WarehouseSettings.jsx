        ...
        <div className="max-w-4xl mx-auto space-y-6">
            <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                    ...
                    <p className="text-gray-500 dark:text-gray-400">Gestiona tus sucursales y proveedores</p>
                </div>
            </div>
                ...
                {loading ? (
                    ...
                    <table className="w-full text-left text-sm text-gray-600 dark:text-gray-300">
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-slate-700">
                            {warehouses.map((w) => (
                                <tr key={w.idAlmacen || w.id} className="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors">
                                    <td className="p-5 font-medium text-gray-900 dark:text-white">{w.nombre}</td>
                                    <td className="p-5">{w.contacto || '-'}</td>
                                    <td className="p-5">{w.direccion || '-'}</td>
                                    <td className="p-5 text-right">
                                        <div className="flex justify-end gap-2">
                                            ...
                                            <button onClick={() => confirmDelete(w)} className="text-red-500 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 p-2 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition">
                                                <Trash2 size={18} />
                                            </button>
                                        </div>
                                    </td>
                            ...
                            {warehouses.length === 0 && (
                                <tr>
                                    <td colSpan="4" className="p-10 text-center text-gray-500 dark:text-gray-400">No hay almacenes registrados</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                    <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md">
                        <div className="p-6 border-b border-gray-100 dark:border-slate-700">
                            <h2 className="text-xl font-bold dark:text-white">{editingWarehouse ? 'Editar Almacén' : 'Nuevo Almacén'}</h2>
                        </div>
            ...
            {itemToDelete && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm relative z-10 overflow-hidden transform transition-all scale-100 p-6 text-center">
                        ...
                        <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">¿Eliminar Almacén?</h3>
